...
public class ApiDaoClips {
    ...
    public String saveSelectiveClips(@Param("api")Api api) {
        StringBuilder query = new StringBuilder("CREATE (n:Api) SET n.systemAlias = #{api.systemAlias}, n.apiName = #{api.apiName}");
    ...
    public String saveOrUpdateSelective(@Param("api") Api api) {
        StringBuilder query = new StringBuilder("MERGE (n:Api {systemAlias : #{api.systemAlias, apiName : #{api.apiName}) SET n.apiName = #{api.apiName}");
        if(!StringOperation.isEmptyOrNull(api.getOthers())) query.append(", n.others = #{api.others}");
        return query.toString();
    }
}
